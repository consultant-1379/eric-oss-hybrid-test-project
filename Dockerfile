...
ARG USER_NAME="eric-oss-hybrid-test-project"
RUN echo "$USER_ID:x:$USER_ID:0:An Identity for $USER_NAME:/nonexistent:/bin/false" >>/etc/passwd

ARG JAR_FILE
ADD target/${JAR_FILE} eric-oss-hybrid-test-project-app.jar
...
CMD ["/bin/sh", "-c", "java ${JAVA_OPTS} -Dcom.sun.management.jmxremote=true \
    ...
    -jar eric-oss-hybrid-test-project-app.jar"]
...
LABEL \
    org.opencontainers.image.title=eric-oss-hybrid-test-project-jsb \
